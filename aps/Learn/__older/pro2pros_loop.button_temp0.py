...
from k3.vis3 import *
from k3.Train_app.Sq_ldr_interval_tester5_modified.Main import get_similarity
from k3.Learn.main import Main6_Output_Object
from k3.Learn.clusters import Clusters,threshold_img
...
if False:
	...
	for i in rlen(I['cluster_avg_img']):
		...
		mci(img,title='img',scale=4*4);#spause();time.sleep(30/1000.)
		mci(world_img,title='world_img',scale=4);spause();time.sleep(30/1000.)
...
if True:
	...
	for i in range(30*3):
		...
		for j in range(20):
			...
			cb(time.time()-t,'seconds to find cluster')
			r0 = r[0]
