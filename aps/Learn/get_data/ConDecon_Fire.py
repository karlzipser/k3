...
def setup(P):
    ...
    for a in activation_folders:
        ...
        E = h5r(files[0])
        if P['type'][1] in ' '.join(list(E.keys())):
    ...
    for r in list(_Runs.keys()):
        ...
        Runs[r]['original_timestamp_data'] = \
            {'path':opj(opjD('Data'),list(H['paths'].keys())[0],r,'original_timestamp_data.h5py'),'data':None}

        Runs[r]['flip_images'] = \
            {'path':opj(opjD('Data'),list(H['paths'].keys())[0],r,'flip_images.h5py'),'data':None}

        Runs[r]['left_timestamp_metadata_right_ts'] = \
            {'path':opj(opjD('Data'),list(H['paths'].keys())[0],r,'left_timestamp_metadata_right_ts.h5py'),'data':None}
