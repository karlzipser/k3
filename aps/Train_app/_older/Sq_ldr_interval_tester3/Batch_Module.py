...
def Batch(_,the_network=None):
	...
	_['ref run path'] = opjD('Data','Network_Predictions_projected',_['the ref run name']+'.net_projections.h5py')
	R = h5r(_['ref run path'])
	_['LDR ref image'] = R['normal'][_['LDR ref index']]
	R.close()
	mi(_['LDR ref image'],d2s(_['LDR ref index'],_['the ref run name']))
	cr(_['ref run path'],ra=0)
