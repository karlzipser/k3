...
import torch
from k3.Train_app.nets.SqueezeNet120_32x1 import SqueezeNet
...
if False:
    ...
    import torch
    #import k3.Train_app.nets.SqueezeNet40
    import k3.Train_app.nets.SqueezeNet120
    A = load_net(most_recent_file_in_folder(opjD('Networks/Sq40_proj_from_scratch_premeta_from_24Dec/weights'),['.infer'],[]))
    B = k3.Train_app.nets.SqueezeNet120.SqueezeNet().cuda()
    C = B.state_dict()
    for k in A.keys():
...
if False:
    ...
    def load_net(path):
        ...
        net = save_data['net']
        return net
    # next, paste in load_net
    import k3.Train_app.nets.SqueezeNet120_32x1
    A = load_net(most_recent_file_in_folder(opjD('Networks/Sq120_ldr_output_4April2019/weights'),['.infer'],[]))
    B = k3.Train_app.nets.SqueezeNet120_32x1.SqueezeNet().cuda()
    C = B.state_dict()
    for k in A.keys():
        if C[k].size() != A[k].size():
            print k,C[k].size(),A[k].size(),C[k].size() == A[k].size()
            a = A[k].size()
            c = C[k].size()
            s = []
            for i in rlen(a):
                s.append(min(a[i],c[i]))
            if 'weight' in k:
                print s[0],s[1],s[2],s[3]
                C[k][:s[0],:s[1],:s[2],:s[3]] = A[k][:s[0],:s[1],:s[2],:s[3]]
            else:
                print s[0]
                C[k][:s[0]] = A[k][:s[0]]
        else:
    ...
    weights = {'net':C}
    torch.save(weights,opjD('Sq120_32x1_from_Sq120.infer'))

    A = load_net('/home/karlzipser/Desktop/Networks/Sq120_ldr_output_32x1/weights/Sq120_32x1_from_Sq120.infer')
    for k in A.keys():
        print k,C[k].size(),A[k].size(),C[k].size() == A[k].size()
