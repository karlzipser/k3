...
# python k3/Train_app/Sq_ldr_interval_tester5/Main.py cluster_list ~/Desktop/cluster_list_25_1st_pass.pkl threshold 0.25
...
while _['ABORT'] == False:
    ...
    for i in indicies:
        ...
        if not (np.abs(ref_index-other_index) < 30*60 and other_name == ref_name):
            continue
        ...
        if np.abs(ref_index-other_index) < 30*60 and other_name == ref_name:
            dindex_value_list.append([np.abs(ref_index-other_index)/30.,value])
    ...
    menu_reminder.message(d2s("\n\nTo start menu:\n\tpython k3/Menu_app/menu2.py path",_['project_path'],"dic P\n\n"))
    ##################################
    #
    ...
    if count_timer.check():
        ...
        if _['display']:
            ...
            figure('dindex_value');clf();pts_plot(dindex_value_list,sym='.');spause()
