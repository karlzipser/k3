...
def Batch(_,the_network=None):
	...
	np_input = np.zeros((D['batch_size'],6,94,168))
	np_target = np.zeros((D['batch_size'],1))
	...
	def _function_fill():
		...
		while ctr < D['batch_size']:
			if True:#try:
				...
				tar = zeros(1)
				tar[0] = d_value
				target_data = torch.unsqueeze(torch.cat((torch.from_numpy(tar)), 0), 0)
				...
				_['frequency_timer'].freq(d2s('train duration =',int(_['duration timer'].time()/60),"minutes\t"))
