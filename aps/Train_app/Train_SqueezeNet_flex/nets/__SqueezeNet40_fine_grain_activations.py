...
class SqueezeNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.post_metadata_features_0 = nn.Sequential(
            Fire(256, 16, 64, 64),
        )
        self.post_metadata_features_1 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
            Fire(128, 32, 128, 128),
            nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
            Fire(256, 48, 192, 192),
    ...
    def forward(self, x, metadata):
        ...
        A['post_metadata_features_0'] = self.post_metadata_features_0(A['pre_metadata_features_metadata'])
        A['post_metadata_features_1'] = self.post_metadata_features_1(A['post_metadata_features_0'])
        A['final_output'] = self.final_output(A['post_metadata_features_1'])
        A['final_output'] = A['final_output'].view(A['final_output'].size(0), -1)
        self.A = A
        return A['final_output']
