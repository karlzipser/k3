...
#from k3.All_Names_Module import *
...
#print("*** Note, k3/teg2/bashrc now does: 'export PYTHONSTARTUP=~/k3/vis2.py' ***")
...
def zrn(c,verify=False,show_only=False):
	f = opjh('k3/scratch/2015/12/scratch_script.py')
...
if False:
	...
	def _zdl(d,dic_show_ends,*alst):
		...
		if len(alst) == 0:
			print("zds(d,dic_show_ends,*alst), but len(alst) == 0")
		list_of_strings_to_txt_file(opjh('k3','zdl.txt'),zdic_to_str(d,alst,False,dic_show_ends).split('\n'))
	...
	def zdl(d,dic_show_ends,*alst):
		...
		list_of_strings_to_txt_file(opjh('k3','zdl.txt'),[out_str,ZD_Dictionary_name]+dic_str.split('\n'))
...
if False:
	...
	def zdprint(*args):
		...
		if True:
			...
			zdset(Args[dic])
			zd(Args[depth])
			txt_lst = txt_file_to_list_of_strings(opjh('k3','zdl.txt'))
			print('\n'.join(txt_lst))
