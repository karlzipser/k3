...
python k3/scripts/ros/publish_preprocessed_data.py  --pub_predictions 0 --step 0 --initial_index 25000

rosplay_menu.py

#python k3/Menu_app/menu2.py --path k3/Cars/j26June2019__/nodes --dic P
python k3/Menu_app/menu2.py --path k3/Cars/j26June2019_rgb_v1_180/nodes --dic P
# 8,12,-1,8,10

#python k3/Cars/j26June2019__/nodes/network_node.py desktop_mode 1
python k3/Cars/j26June2019_rgb_v1_180/nodes/network_node.py desktop_mode 1

python k3/VT_net2__5April2019_2__18April2019_for_speed/main.py

python k3/Menu_app/menu2.py --path k3/VT_net2__5April2019_2__18April2019_for_speed --dic P
"""
...
import torch
import k3.VT_net2__1June2019.default_values as VT_net2__1June2019_default_values
#import flex_network_node
N = default_values.P

N['circle_lifetime'] = VT_net2__1June2019_default_values.P['circle_lifetime']
# python k3/Cars/n26Dec18/nodes/network_node.py desktop_mode 1 display 'camera_input' delay_blank 500 delay_prev 500 delay_now 750
# python k3/Cars/n26Dec18/nodes/network_node.py desktop_mode 1 display 'pre_metadata_features_metadata' delay_now 66
...
if N['use flex']:
    #!/usr/bin/env python
    from k3.vis3 import *
    import k3.Menu_app.menu2 as menu2
    exec(identify_file_str)
    ...
    if not N['use flex']:
        ...
        while not rospy.is_shutdown():
            time.sleep(10)
    import rospy
    import k3.Data_app.collect_flex_data2 as fx
    import roslib
    import std_msgs.msg
    import torch
...
rospy.Subscriber('/bair_car/FLEX', std_msgs.msg.Int32, callback=FLEX__callback)
    ...
    from k3.Train_app.nets.SqueezeNet_flex import SqueezeNet
    ...
    while not rospy.is_shutdown() and N['ABORT'] == False:
        ...
        #try:
            ...
            if network_utils.run.ready(N):
                #cr(6)
                #print(cf("len(network_utils.camera.Q_list)=",len(network_utils.camera.Q_list)),fline())
                if len(network_utils.camera.rgb_v1_list) > 1:
                    #cr(7)
                    #print(fline())
                    ...
                    if True:#!!Q['ready']:
                        ...
                        print(N['t0'],N['tn1'])
                        listoftensors = []
                        listoftensors.append(torch.from_numpy(network_utils.camera.rgb_v1_list[N['t0']]))
                        listoftensors.append(torch.from_numpy(network_utils.camera.rgb_v1_list[N['tn1']]))
                        camera_data = torch.cat(listoftensors, 2)
                        camera_data = camera_data.cuda().float()/255. - 0.5
                        camera_data = torch.transpose(camera_data, 0, 2)
                        camera_data = torch.transpose(camera_data, 1, 2)
                        torch_camera_data = camera_data.unsqueeze(0)
                        #print torch_camera_data.size()
                        ...
                        #
                        ###############################################
                        #print torch_camera_data.size(),torch_camera_data.size()
                        network_utils.run.step(torch_camera_data,torch_metadata,N)

                        ###############################################
                        #
                        if False:#Arguments['display']:
