		...
		while ctr < D['batch_size']:
			if True:#try:
				...
				if False:
					...
					#rv = range(8,91,9)
					rv = _['prediction_range'] #range(10)
					sv = array(sv)[rv]
					mv = array(mv)[rv]
	...
	def _function_display():
		...
		if _['print_timer'].check():
			...
			for i in [0]:#range(_['BATCH_SIZE']):
				...
				figure(fname(_['project_path'])+' steer '+_['start time'],figsize=P['figure size'])
				clf()
				plt.title(d2s(i))
				...
				if False:
					figure('metadata '+_['start time'],figsize=(1,6));clf()
					plot(mv[-10:,0,0],'r.-')
					plt.title(d2s(bm,i))
				spause()
				_['print_timer'].reset()
			...
			if False:
				figure('dm_ctrs '+_['start time']);clf();plot(dm_ctrs,'.-');xlim(0,100)
			_['dm_ctrs'] = dm_ctrs
		...
		if _['loss_timer'].check() and len(_['LOSS_LIST_AVG'])>5:
			...
			figure(fname(_['project_path'])+' LOSS_LIST_AVG '+_['start time'],figsize=P['figure size']);clf();plot(_['LOSS_LIST_AVG'][-q:],'.')
