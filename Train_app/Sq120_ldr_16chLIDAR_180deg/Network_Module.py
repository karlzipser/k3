...
import torch
#from k3.Train_app.nets.SqueezeNet40_global_A import SqueezeNet
from k3.Train_app.nets.SqueezeNet120 import SqueezeNet as SqueezeNet_zed
from k3.Train_app.nets.SqueezeNet120_16chLIDAR import SqueezeNet as SqueezeNet120_16chLIDAR
...
def Pytorch_Network(P,network_class='zed'):
    ...
    """
    GPUs = gpu_stats(num=100)
    if GPUs[0]['util'] < 5 and GPUs[1]['util'] < 5:
        P['GPU'] = random.choice([0,1])
    else:
        P['GPU'] = GPUs['most_free']
    for i in range(20):
        clp("Using GPU",P['GPU'])
