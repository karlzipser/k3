...
if False:#__name__ == '__main__':
	...
	if False:
		from k3.drafts.runs_with_points import *
...
def find_runs_with_topic(topic,top,filetype):
	if filetype == 'h5py':
		pattern = 'original_*'
	elif filetype == 'bag':
		pattern = '*.bag'
...
def make_folder_of_links_to_runs_with_topic(folder_path,topic,top,filetype):
	...
	runs_with_topic = find_runs_with_topic(topic,top,filetype)
...
if False:
	...
	for f in Runs.keys():
		for t in Runs[f].keys():
			runs = find_runs_with_topic(t,opjm('rosbags'),f)
			for r in runs:
				Runs[f][t].append(fname(r))
