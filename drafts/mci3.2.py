...
def get_args():
    ...
    aa(
        ...
        type=str,
        required=True,
        default=fname(pname(__file__)) #fnamene(__file__), 
...
#args.project_name = fnamene(__file__)



def __save_L(L):
    so(L,opjh('Logs',args.project_name,args.topic,d2p(args.project_name,args.topic,str(int(time.time())),'log')))
...
def setup_L():
    try:
        ...
        f = most_recent_file_in_folder(opjh('Logs',args.project_name,args.topic),str_elements=[args.project_name])
        L = lo(f)
        del_lst = []
