...
for y in sorted(O['Ping']):
	#print y
	for m in sorted(O['Ping'][y]):
		#print m
		for d in sorted(O['Ping'][y][m]):
			clp(m,'/',d,'/',y,s0='')
			for h in sorted(O['Ping'][y][m][d]):
				#print h
				for mn in sorted(O['Ping'][y][m][d][h]):
					#print mn
					for s in sorted(O['Ping'][y][m][d][h][mn]):
						O['Ping'][y][m][d][h][mn][s]
						#print y,m,d,h,mn,s
						txt = O['Ping'][y][m][d][h][mn][s]['text']
						if O['Ping'][y][m][d][h][mn][s]['is_from_me']:
							c = 'white'
						else:
							c = 'blue'
						cprint(txt,c)
